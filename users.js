...
function startRecursive() {
	count +=10
	let arrayPromise = []
	users.slice(count - 10,count).map(item => arrayPromise.push(processUser(item.id)))
	return Promise.all(arrayPromise)
		.then(data => data.reduce((prev, curr) => {sum += curr}, 0))
		.then(() => console.log('---------------------'))
		.then(() => count < 100 ? startRecursive() : sum)
